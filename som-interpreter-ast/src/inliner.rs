...
pub trait PrimMessageInliner {
    ...
    fn adapt_var_or_args_coords_from_inlining(
        &self,
        up_idx: usize,
        idx: usize,
        vars_or_args_fetcher_func: fn(&AstScopeCtxt) -> usize,
    ) -> (u8, u8);
...
impl PrimMessageInliner for AstMethodCompilerCtxt<'_> {
    ...
    fn parse_expression_with_inlining(&mut self, expression: &Expression) -> AstExpression {
        let expr = match expression {
            Expression::Block(blk) => {
                ...
                let new_blk_ptr = GCRef::<AstBlock>::alloc(new_blk, self.gc_interface); // could we just adapt the old block instead of allocating?
            ...
            | Expression::NonLocalVarWrite(_, idx, _) => {
                let up_idx = match expression {
                    ...
                    Expression::NonLocalVarRead(up_idx, ..)
                    | Expression::NonLocalVarWrite(up_idx, ..) => *up_idx,
                    _ => unreachable!(),
                };

                let (new_up_idx, new_idx) = self.adapt_var_or_args_coords_from_inlining(
                    up_idx,
                    *idx,
                    AstScopeCtxt::get_nbr_locals,
                );

                match new_up_idx {
                    0 => match expression {
                        Expression::LocalVarRead(..) | Expression::NonLocalVarRead(..) => {
                            AstExpression::LocalVarRead(new_idx)
                        }
                        Expression::LocalVarWrite(.., expr)
                        | Expression::NonLocalVarWrite(.., expr) => AstExpression::LocalVarWrite(
                            new_idx,
                            Box::new(self.parse_expression_with_inlining(expr)),
                        ),
                        _ => unreachable!(),
                    },
                    _ => match expression {
                        Expression::NonLocalVarRead(..) => {
                            AstExpression::NonLocalVarRead(new_up_idx, new_idx)
                        }
                        Expression::NonLocalVarWrite(.., expr) => AstExpression::NonLocalVarWrite(
                            new_up_idx,
                            new_idx,
                            Box::new(self.parse_expression_with_inlining(expr)),
                        ),
            ...
            Expression::ArgRead(up_idx, idx) | Expression::ArgWrite(up_idx, idx, _) => {
                ...
                match expression {
                    ...
                    Expression::ArgWrite(.., expr) => AstExpression::ArgWrite(
                        new_up_idx,
                        new_idx,
                        Box::new(self.parse_expression_with_inlining(expr)),
                    ),
            ...
            Expression::Exit(expr, scope) => {
                ...
                let adjust_scope_by = self
                    .scopes
                    .iter()
                    .rev()
                    .take(*scope)
                    .filter(|e| e.is_getting_inlined)
                    .count();
    ...
    fn inline_block(&mut self, blk: &Block) -> AstBody {
        self.scopes
            .push(AstScopeCtxt::init(blk.nbr_params, blk.nbr_locals, true));
        ...
        let inlined_block = AstBody {
            exprs: blk
                ...
                .exprs
                .iter()
                .map(|e| self.parse_expression_with_inlining(e))
                .collect(),
        };
        ...
        self.scopes
            .last_mut()
            .unwrap()
            .add_nbr_args(nbr_params_post_inlining);
        self.scopes
            .last_mut()
            .unwrap()
            .add_nbr_locals(nbr_locals_post_inlining);
    ...
    fn adapt_block_after_outer_inlined(&mut self, blk: &Block) -> AstBlock {
        ...
        let exprs: Vec<AstExpression> = blk
            .body
            .exprs
            .iter()
            .map(|og_expr| self.parse_expression_with_inlining(og_expr))
            .collect();

        let (nbr_params, nbr_locals) = {
            let outer_blk_scope = self.scopes.last().unwrap();
            (
                outer_blk_scope.get_nbr_args() as u8,
                outer_blk_scope.get_nbr_locals() as u8,
            )
        };
    ...
    fn adapt_var_or_args_coords_from_inlining(
        ...
        let new_idx = {
            let up_idx_of_scope_var_will_end_up_into = self
                .scopes
                .iter()
                .rev()
                .skip(up_idx)
                ...
                .count()
                + up_idx; // if we're accessing a var into a scope that's getting inlined, then its idx may have changed due to the inlining.
    ...
    fn inline_if_true_if_false(
        ...
        Some(InlinedNode::IfTrueIfFalseInlined(
            if_true_if_false_inlined_node,
        ))
    }

    fn inline_while(&mut self, msg: &ast::Message, expected_bool: bool) -> Option<InlinedNode> {
        let (cond_blk, body_blk) = match (&msg.receiver, msg.values.first()) {
            (Expression::Block(cond_blk), Some(Expression::Block(body_blk))) => {
                (cond_blk, body_blk)
            }
            _ => return None,
        };
