...
impl<'a> AstMethodCompilerCtxt<'a> {
    ...
    pub fn parse_message_with_func(
        ...
        // TODO: this is matching a HACK in the BC. This is not needed, and is probably only a rare and minor perf boost.
        if msg.signature == "ifNotNil:" {
            return AstExpression::BinaryDispatch(Box::new(AstBinaryDispatch {
                dispatch_node: AstDispatchNode {
                    signature: self.interner.intern("ifTrue:"),
                    receiver: {
                        AstExpression::UnaryDispatch(Box::new(AstUnaryDispatch {
                            dispatch_node: AstDispatchNode {
                                signature: self.interner.intern("notNil"),
                                receiver: expr_parsing_func(self, &msg.receiver),
                                inline_cache: None,
                            },
                        }))
                    },
                ...
                arg: {
                    ...
                    expr_parsing_func(self, msg.values.first().unwrap())
                },
            }));
        } else if msg.signature == "ifNil:" {
            return AstExpression::BinaryDispatch(Box::new(AstBinaryDispatch {
                dispatch_node: AstDispatchNode {
                    signature: self.interner.intern("ifTrue:"),
                    receiver: {
                        AstExpression::UnaryDispatch(Box::new(AstUnaryDispatch {
                            dispatch_node: AstDispatchNode {
                                signature: self.interner.intern("isNil"),
                                receiver: expr_parsing_func(self, &msg.receiver),
                                inline_cache: None,
                            },
                        }))
        ...
        } else if msg.signature == "ifNil:ifNotNil:" {
            return AstExpression::TernaryDispatch(Box::new(AstTernaryDispatch {
                dispatch_node: AstDispatchNode {
                    signature: self.interner.intern("ifTrue:ifFalse:"),
                    receiver: {
                        AstExpression::UnaryDispatch(Box::new(AstUnaryDispatch {
                            dispatch_node: AstDispatchNode {
                                signature: self.interner.intern("isNil"),
