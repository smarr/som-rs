...
use crate::specialized::trivial_methods::{
...
impl<'a> AstMethodCompilerCtxt<'a> {
    pub fn get_method_kind(
        method: &ast::MethodDef,
        class: Option<GCRef<Class>>,
        ...
        match method.signature.as_str() {
            "ifTrue:" => MethodKind::Specialized(MethodKindSpecialized::If(IfNode {
                expected_bool: true,
            })),
            "ifFalse:" => MethodKind::Specialized(MethodKindSpecialized::If(IfNode {
                expected_bool: false,
            })),
            "ifTrue:ifFalse:" => {
                MethodKind::Specialized(MethodKindSpecialized::IfTrueIfFalse(IfTrueIfFalseNode {}))
            }
            "whileTrue:" => MethodKind::Specialized(MethodKindSpecialized::While(WhileNode {
                expected_bool: true,
            })),
            "whileFalse:" => MethodKind::Specialized(MethodKindSpecialized::While(WhileNode {
                expected_bool: false,
            })),
            ...
            _ => match method.body {
                ...
                MethodBody::Body { .. } => {
                    ...
                    if let Some(trivial_method_kind) =
                        AstMethodCompilerCtxt::make_trivial_method_if_possible(&ast_method_def)
                    {
                        trivial_method_kind
                    } else {
        ...
        match method_def.body.exprs.first()? {
            AstExpression::LocalExit(expr) => {
                match expr.as_ref() {
                    AstExpression::Literal(lit) => {
                        Some(MethodKind::TrivialLiteral(TrivialLiteralMethod {
                            literal: lit.clone(),
                        })) // todo avoid clone by moving code to previous function tbh
                    }
                    AstExpression::GlobalRead(global) => {
                        Some(MethodKind::TrivialGlobal(TrivialGlobalMethod {
                            global_name: *global.clone(),
                        }))
                    }
                    AstExpression::FieldRead(idx) => {
                        Some(MethodKind::TrivialGetter(TrivialGetterMethod {
                            field_idx: *idx,
                        }))
                    }
                    _ => None,
            ...
            AstExpression::FieldWrite(idx, expr) => match expr.as_ref() {
                AstExpression::ArgRead(0, 1) => {
                    Some(MethodKind::TrivialSetter(TrivialSetterMethod {
                        field_idx: *idx,
                    }))
                }
                _ => None,
    ...
    pub fn parse_method_def(
        ...
        let (body, locals_nbr) = match &method_def.body {
            ...
            MethodBody::Body {
                ...
                (
                    ctxt.parse_body(body),
                    ctxt.scopes.last().unwrap().get_nbr_locals() as u8,
                )
            }
        };
    ...
    pub fn parse_expression(&mut self, expr: &Expression) -> AstExpression {
        match expr.clone() {
            Expression::GlobalRead(global_name) => {
                self.global_or_field_read_from_superclass(global_name)
            }
            Expression::GlobalWrite(global_name, expr) => {
                self.resolve_global_write_to_field_write(&global_name, expr.as_ref())
            }
            Expression::LocalVarRead(idx) => AstExpression::LocalVarRead(idx as u8),
            Expression::NonLocalVarRead(scope, idx) => {
                AstExpression::NonLocalVarRead(scope as u8, idx as u8)
            }
            Expression::ArgRead(scope, idx) => AstExpression::ArgRead(scope as u8, idx as u8),
            Expression::LocalVarWrite(a, b) => {
                AstExpression::LocalVarWrite(a as u8, Box::new(self.parse_expression(b.as_ref())))
            }
            Expression::NonLocalVarWrite(a, b, c) => AstExpression::NonLocalVarWrite(
                a as u8,
                b as u8,
                Box::new(self.parse_expression(c.as_ref())),
            ),
            Expression::ArgWrite(a, b, c) => AstExpression::ArgWrite(
                a as u8,
                b as u8,
                Box::new(self.parse_expression(c.as_ref())),
            ),
            ...
            Expression::Exit(a, b) => match b {
                ...
                _ => AstExpression::NonLocalExit(
                    Box::new(self.parse_expression(a.as_ref())),
                    b as u8,
                ),
            },
    ...
    pub fn parse_block(&mut self, blk: &ast::Block) -> AstBlock {
        self.scopes
            .push(AstScopeCtxt::init(blk.nbr_params, blk.nbr_locals, false));
    ...
    pub fn parse_message_with_func(
        ...
        if msg.receiver == Expression::GlobalRead(String::from("super")) {
            return AstExpression::SuperMessage(Box::new(AstSuperMessage {
                super_class: self.class.unwrap().super_class.clone().unwrap_or_else(|| {
                    panic!("no super class set, even though the method has a super call?")
                }),
                signature: msg.signature.clone(),
                values: msg
                    .values
                    .iter()
                    .map(|e| expr_parsing_func(self, e))
                    .collect(),
            }));
        }
    ...
    pub fn resolve_global_write_to_field_write(
        &mut self,
        name: &String,
        expr: &Expression,
    ) -> AstExpression {
