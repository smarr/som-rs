...
use som_interpreter_bc::frame::{Frame, FrameDirectAccess};
...
fn frame_stack_accesses() {
    ...
    let mut frame_ptr = Frame::alloc_from_method(method_ref,
                                                 &[Value::Double(1000.0), Value::SYSTEM],
                                                 GCRef::default(),
                                                 &mut universe.gc_interface);

    assert_eq!(frame_ptr.stack_len(), 0);
    frame_ptr.stack_push(Value::Boolean(true));
    assert_eq!(frame_ptr.stack_len(), 1);
    
    assert_eq!(frame_ptr.stack_pop().as_boolean(), Some(true));
    assert_eq!(frame_ptr.stack_len(), 0);

    frame_ptr.stack_push(Value::Integer(10000));
    frame_ptr.stack_push(Value::Double(424242.424242));
    assert_eq!(frame_ptr.stack_len(), 2);
    
    assert_eq!(frame_ptr.stack_last().as_double(), Some(424242.424242));
    assert_eq!(frame_ptr.stack_last_mut().as_double(), Some(424242.424242));

    assert_eq!(frame_ptr.stack_nth_back(0).as_double(), Some(424242.424242));
    assert_eq!(frame_ptr.stack_nth_back(1).as_integer(), Some(10000));
}
...
fn frame_stack_split_off() {
    ...
    frame_ptr.stack_push(Value::Integer(10000));
    frame_ptr.stack_push(Value::Double(424242.424242));
    frame_ptr.stack_push(Value::NIL);
    frame_ptr.stack_push(Value::INTEGER_ONE);
    frame_ptr.stack_push(Value::Boolean(true));

    assert_eq!(frame_ptr.stack_len(), 5);
    
    let two_last = frame_ptr.stack_n_last_elements(2);

    assert_eq!(two_last, vec![Value::INTEGER_ONE, Value::Boolean(true)]);

    assert_eq!(frame_ptr.stack_len(), 3);
    assert_eq!(frame_ptr.stack_last(), &Value::NIL);
}
