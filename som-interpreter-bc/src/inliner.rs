...
pub trait PrimMessageInliner {
    ...
    fn inline_if_possible(&self, ctxt: &mut dyn InnerGenCtxt, mutator: &mut BumpAllocator<SOMVM>) -> Option<()>;
    /// Inlines an expression. If this results in a PushBlock, calls `inline_last_push_block_bc(...)` to inline the block.
    fn inline_expression(&self, ctxt: &mut dyn InnerGenCtxt, expression: &ast::Expression, mutator: &mut BumpAllocator<SOMVM>) -> Option<()>;
    ...
    fn inline_last_push_block_bc(&self, ctxt: &mut dyn InnerGenCtxt, mutator: &mut BumpAllocator<SOMVM>) -> Option<()>;
    /// Inlines a compiled block into the current scope.
    fn inline_compiled_block(&self, ctxt: &mut dyn InnerGenCtxt, block: &BlockInfo, mutator: &mut BumpAllocator<SOMVM>) -> Option<()>;
