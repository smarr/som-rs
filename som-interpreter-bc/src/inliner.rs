            ...
            for block_bc in body {
                match block_bc {
                    ...
                    Bytecode::PushArgument(up_idx, idx) => {
                        match (up_idx, idx) {
                            (0, 0) => ctxt.push_instr(Bytecode::PushArgument(0, 0)),
                            _ => ctxt.push_instr(Bytecode::PushArgument(*up_idx - 1, *idx))
                        }
                    ...
                    Bytecode::ReturnNonLocal => {
                        ...
                        // ReturnNonLocal should eventually get a scope as argument, which will make fixing this case super simple ("ReturnNonLocal(scope - 1), or ReturnLocal if scope is 1")
