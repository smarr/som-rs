...
fn as_string(interpreter: &mut Interpreter, universe: &mut UniverseBC) {
    ...
    {
        interpreter.stack.push(Value::String(GCRef::<String>::alloc(value, universe.allocator.as_mut())));
        return;
    }
}
...
fn to_do(interpreter: &mut Interpreter, universe: &mut UniverseBC) {
    ...
    let new_blk = blk.to_obj().make_equivalent_with_no_return(universe.allocator.as_mut());
    ...
    for i in (start..=end).rev() {
        interpreter.push_block_frame(new_blk, vec![Value::Block(new_blk), Value::Integer(i)], universe.allocator.as_mut());
    }
...
fn times_repeat(interpreter: &mut Interpreter, universe: &mut UniverseBC) {
    ...
    let new_blk = blk.to_obj().make_equivalent_with_no_return(universe.allocator.as_mut());
    for _ in 1..=n {
        interpreter.push_block_frame(new_blk, vec![Value::Block(new_blk)], universe.allocator.as_mut()); // NB: this doesn't take the index as an argument
    }
