        ...
        match identifier().parse(input, Rc::clone(&genctxt)) {
            ...
            Some((name, input, genctxt)) => {
                genctxt.borrow().get_local(&name) // TODO known bug: checking params of ANY scope before non locals of any scope can mess up if the non local is closer, since it should have priority.
                    ...
                    .or(genctxt.borrow().get_param(&name).and_then(|(_, scope)| Some((Expression::ArgRead(name.clone(), scope), input, Rc::clone(&genctxt)))))
                    .or(genctxt.borrow().get_non_local(&name).and_then(|(idx, scope)| Some((Expression::NonLocalVarRead(scope, idx), input, Rc::clone(&genctxt)))))
                    .or((name.as_str() == "self").then_some((Expression::ArgRead(name.clone(), 0), input, Rc::clone(&genctxt)))) // bit lame we hardcode self i thiiink?
        ...
        match identifier()
            ...
            Some(((name, expr), input, genctxt)) => {
                ...
                genctxt.borrow().get_local(&name).and_then(|idx| Some((Expression::LocalVarWrite(idx, Box::new(expr.clone())), input, Rc::clone(&genctxt))))
                    .or(genctxt.borrow().get_param(&name).and_then(|(_, scope)| Some((Expression::ArgWrite(name.clone(), scope, Box::new(expr.clone())), input, Rc::clone(&genctxt)))))
                    ...
                    .or((name.as_str() == "self").then_some((Expression::ArgWrite(name.clone(), 0, Box::new(expr.clone())), input, Rc::clone(&genctxt)))) // bit lame i thiiink?
